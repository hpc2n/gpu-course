...
int main(int argc, char const **argv)
{
    ...
    cudaStream_t *stream = (cudaStream_t *) malloc(count*sizeof(cudaStream_t));
    ...
    for (int i = 0; i < count; i++) {
        ...
        CHECK_CUDA_ERROR(
            cudaHostRegister(
                A[i], n*ldA*sizeof(double), cudaHostRegisterDefault));
        CHECK_CUDA_ERROR(
            cudaHostRegister(
                B[i], n*ldB*sizeof(double), cudaHostRegisterDefault));
        CHECK_CUDA_ERROR(
            cudaHostRegister(
                C[i], n*ldC*sizeof(double), cudaHostRegisterDefault));
    ...
    for (int i = 0; i < count; i++) {
        ...
        CHECK_CUDA_ERROR(
            cudaMemcpyAsync(_A[i], A[i], n*ldA*sizeof(double),
                cudaMemcpyHostToDevice, stream[i]));
        CHECK_CUDA_ERROR(
            cudaMemcpyAsync(_B[i], B[i], n*ldB*sizeof(double), 
                cudaMemcpyHostToDevice, stream[i]));
        
        // set CUDA stream for the cuBLAS call
        
        CHECK_CUBLAS_ERROR(cublasSetStream(handle, stream[i]));
        ...
        CHECK_CUDA_ERROR(
            cudaMemcpyAsync(C[i], _C[i], n*ldC*sizeof(double),
    ...
    for (int i = 0; i < count; i++) {
        ...
        CHECK_CUDA_ERROR(cudaHostUnregister(A[i]));
        CHECK_CUDA_ERROR(cudaHostUnregister(B[i]));
        CHECK_CUDA_ERROR(cudaHostUnregister(C[i]));
    ...
    free(A); free(B); free(C); free(p); free(stream);
