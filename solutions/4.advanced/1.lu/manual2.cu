...
///
void blocked_lu(
    cublasHandle_t handle, int block_size, int n, int ldA, double *A)
{
    ...
    // allocate a page-locked buffer for diagonal blocks
    double *T; int ldT = DIVCEIL(block_size, 8)*8;
    CHECK_CUDA_ERROR(cudaMallocHost(&T, block_size*ldT*sizeof(double)));
    ...
    for (int i = 0; i < block_count; i++) {
        ...
        // blocks[i][i] -> T
        CHECK_CUDA_ERROR(
            cudaMemcpy2D(T, ldT*sizeof(double), blocks[i][i], ldA*sizeof(double), 
                dsize*sizeof(double), dsize, cudaMemcpyDeviceToHost));
        
        // T <- (L,U)
        simple_lu(dsize, ldT, T);
        
        // T -> blocks[i][i]
        CHECK_CUDA_ERROR(
            cudaMemcpy2D(blocks[i][i], ldA*sizeof(double), T, ldT*sizeof(double),
                dsize*sizeof(double), dsize, cudaMemcpyHostToDevice));
    ...
    // free the page-locked buffer
    CHECK_CUDA_ERROR(cudaFreeHost(T));
}
